	...
	song.save((err, songStored) => {
		...
		if (err) {
			...
			if (!songStored) {
				...
				res.status(404).send({message: 'No se ha guardado el registro'});
			}else{

				res.status(200).send({data: songStored});
			}
		}
	});
}
...
function updateSong(req, res){
	...
	Song.findByIdAndUpdate(songId, update, (err, songUpdated) =>{
		if (err) {
			...
			rest.status(500).send({message: 'Error en el servidor'});
		}else{

			if (!songUpdated) {

				rest.status(404).send({message: 'No se ha actualizado la cancion'});
			}else{

				res.status(200).send({data: songUpdated});
			}
		}
	});
}
	...
	Song.findByIdAndRemove(songId, (err, songDeleted) => {
		...
		if (err) {
			...
			if (!songDeleted) {
				...
				res.status(200).send({data: songDeleted});
			}
		}
	});
}
