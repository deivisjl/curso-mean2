	...
	album.save((err, albumStored)=>{
		...
		if (err) {
			...
			if (!albumStored) {
				...
				res.status(404).send({message: 'Error al guardar el registro'});
			}else{

				res.status(200).send({data: albumStored});
			}
		}
	});
}
	...
	Album.findByIdAndUpdate(albumId, update, (err, albumUpdated) =>{
		...
		if (err) {
			...
			if (!albumUpdated) {
				...
				res.status(200).send({data: albumUpdated});
			}
		}
	});
}
							...
							Song.find({album: albumRemoved._id}).remove((err, songRemoved)=>{
								...
								if (err) {
									...
									if (!albumRemoved) {
										...
										res.status(200).send({data: albumRemoved});
									}
								}
							});
						}
